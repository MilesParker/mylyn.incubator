...
import org.eclipse.mylyn.modeling.context.AbstractModelingContextTest;
 ...
 * @author Miles Parker
 */
public class EmfUIBridgeTest extends AbstractModelingContextTest {

	private DiagramUiEditingMonitor monitor;

	private EcoreGmfDomainBridge structureBridge;

	private IJavaProject emfProject;
	...
	public void testModification() throws Exception {
		emfProject.open(new NullProgressMonitor());
		IProject project = emfProject.getProject();
		...
		ResourceSet resourceSet = ed.getEditingDomain().getResourceSet();
		EObject eObject = resourceSet.getResources().get(0).getContents().get(0);
		if (eObject instanceof EPackage) {
			p = (EPackage) eObject;
		} else {
			eObject = resourceSet.getResources().get(1).getContents().get(0);
			p = (EPackage) eObject;
		}
